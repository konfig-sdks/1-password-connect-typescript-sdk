...
import { readableStreamToString, OnePasswordConnectError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new OnePasswordConnectError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new OnePasswordConnectError(e, e.response?.data)
                }
                if (innerError instanceof OnePasswordConnectError) {
                    // Got "OnePasswordConnectError" from the above try block
                    throw innerError;
                }
